"""
Модуль для дифференциального кодирования (Differential Pulse Code Modulation) DC‐коэффициентов JPEG.

Класс DCDifferentialCodec позволяет:
- выполнять прямое дифференциальное кодирование DC‐коэффициентов (запись разностей);
- выполнять обратное декодирование (восстановление исходных DC‐коэффициентов).
"""
...
class DCDifferentialCodec:
    """
    Класс для дифференциального кодирования и декодирования последовательности DC‐коэффициентов.

    Методы:
        - encode: преобразует список DC‐коэффициентов в список разностей соседних значений;
        - decode: восстанавливает оригинальные DC‐коэффициенты из списка разностей.
    """
