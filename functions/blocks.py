"""
Модуль разбиения и сборки 2D-изображения (одноканального) на блоки размером N×N.

Класс BlockSplitter позволяет:
- разбивать непрерывный байтовый буфер изображения (ширина×высота) на блоки N×N с паддингом;
- собирать изображение обратно из списка блоков, обрезая до исходных размеров.
"""
...
class BlockSplitter:
    """
    Класс для разбиения изображения на блоки N×N и обратной сборки.
